...
## メロディ直送の診断用ミュート（2026-09-17）

- melody-followに `--mute-melody` を追加。受信callbackは維持し、forward_pendingの転送先だけNoneにする。
- CLI統合テストで第三・第四MVP×通常・debug表示の4通りについて、
  ミュート前後の推定コード・Activeコード・伴奏MIDIイベント列が完全一致することを確認。
  メロディのNote On/Offは0件、コンピングとベースは送信され、debug送信件数はMelody=0。
- 全132テスト中129件成功、従来と同じ環境条件で3件スキップ。
- 通常モードのスルー、play/follow、UNO Q関連は変更なし。今回の聴感評価は未実施。

## 第五MVP jazzスタイル（2026-09-17）

- melody-followにstyle basic/jazzとseedを追加。既定basicの送信イベント列が従来と一致することを確認。
- jazzは4拍以上保持＋小節頭変更を原則とし、強い根拠に限り3拍目で変更。
  ルート省略ボイシング、4パターン、密度抑制、音域分離、近接ベースと予約先への接続を実装。
- 全142テスト中139件成功、従来と同じ環境条件で3件スキップ。
- 追加10テストでseed再現、basic互換、変更頻度抑制、パターン巡回、候補6コード全組合せの
  ボイシング移動7半音以内、ベース範囲36〜50・終端接続・3音以上の音高使用、
  予約先ルートへの実出力、密度抑制、ミュートと実送信ログを確認。
- 疑似単音入力→CLI→実GS音源でjazz/seed1/mute-melody/debug-accompを4小節実行。
  C→F→Dm、パターンB→C→A→D、Melody=0 / Comping=21 / Bass=15、遅延スキップ0件。
  その確認でベースの同音反復が多い生成例を見つけ、反復ペナルティを強めて最終調整。
...
## A/B録音の拍同期（2026-09-17）

- record-melodyにcount-in-bars、click、クリック用output指定を追加。
  4/4のカウントイン完了後の小節頭をtime 0とし、入力の開始音へは寄せ直さない。
  GM打楽器ch10の高/低ウッドブロックで強拍を区別。録音開始の1拍目も鳴らす。
- 追加3テストで2小節後のtime 0、120 BPMの0.5秒間隔、強拍区別、カウントイン入力除外、
  VelocityとNote Off維持、basic/jazz入力一致、機器起動待ち19秒の位相非依存、click継続を検証。
- 全151テスト中148件成功、既存と同じ環境条件で3件スキップ。
...
## Loopian Phase 1 音域端のフレーズ再配置（2026-09-17）

- 実鍵盤で報告されたC6→D5の単音オクターブ折返しを廃止。
  既定音域48〜96、range-low/range-high追加（note-min/note-max互換）。
- 音域端ではRANGE_LIMITとして発音中の音とペダルを解放し、60msの発音区切り後に
  中域のコードルートから再開。再配置以外ではUP/DOWNを保持。
  Note On間隔400ms以上はINPUT_PAUSEとして再配置。大跳躍は方向としてのみ使用。
- 5テスト追加と既存テスト更新。100音の連続UP/DOWN×3コード×4音域、
  実MIDIイベント列での境界の区切り・短いタップの解放・ペダル復元・音順・ログ、
  再開前のコード変更・停止時予約キャンセル、CLIの新旧引数を検証。
- 全179件中176件成功、既存の環境依存3件スキップ。git diff --checkも成功。
