"""Runtime checks shared by UNO Q launch/deploy/doctor commands."""
...
RUNNING_WORDS = {"running", "starting", "started", "active"}
STOPPED_WORDS = {"stopped", "stop", "exited", "inactive", "created", "idle"}


def relay_probe(path: Path, command: bytes = b"PING\n", timeout: float = 1.0):
    """Return (reachable, detail), without taking relay output ownership."""
    try:
...
def _app_rows(value):
    if isinstance(value, list):
        ...
        for key in ("apps", "result", "data"):
            if key in value:
                yield from _app_rows(value[key])


def app_lab_status(app: Path):
    """Return (running|stopped|unknown, detail) from the official App CLI."""
    try:
    ...
    if result.returncode != 0:
        return "unknown", (result.stderr or output).strip() or f"exit {result.returncode}"
    try:
        rows = list(_app_rows(json.loads(output)))
    except (json.JSONDecodeError, TypeError) as error:
        return "unknown", f"unrecognized app list JSON: {error}"
    target = str(app.expanduser().absolute())
    basename = app.name.casefold()
    matches = []
    for row in rows:
        strings = [str(value) for value in row.values() if isinstance(value, (str, int))]
        if target in strings or any(value.casefold() == basename for value in strings):
            matches.append(row)
    if not matches:
    ...
    if normalized in RUNNING_WORDS:
        return "running", status
    if normalized in STOPPED_WORDS:
        return "stopped", status
    return "unknown", f"status={status}; row={row}"


def remove_stale_socket(path: Path, app: Path, app_stopped: bool = False) -> bool:
    """Remove only an unreachable socket directly inside a definitely stopped App."""
    try:
